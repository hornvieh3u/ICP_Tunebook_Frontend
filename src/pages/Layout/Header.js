    ...
    <>
      <div className={`absolute z-40 bg-white px-4 flex justify-between h-16 flex flex-row items-center w-full`}>
        ...
        <div className="flex flex-row justify-end items-center gap-3">
          ...
          <p className="text-darkblue-800 text-[16px] font-bold cursor-pointer" onClick={() => {history.push("/app/sessions")}}>Sessions</p>
          {
            auth.isLoggedIn ?
            (<p className="text-darkblue-800 text-[16px] font-bold cursor-pointer" onClick={() => {logout()}}>Log out</p>) : 
            (<p className="text-darkblue-800 text-[16px] font-bold cursor-pointer" onClick={() => {history.push("/app/login")}}>Log In</p>)
          }
        </div>
      </div>
    </>
