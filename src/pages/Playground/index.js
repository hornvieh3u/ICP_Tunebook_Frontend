...
import auth, { UpdateInfo, SetTitle, SetOrgTunes, SetCurrentTune } from '../../store/reducers/auth.js';
...
function Playground() {
    ...
    const {user, currentTune, isLoggedIn} = useSelector((state) => (state.auth));
    ...
    const addToTuneBook = async() => {
        ...
        if (!isLoggedIn) {
            alert("warning", "Please log in now!");
